 ...
 * shape {startMs, endMs, text?, ...}. The caller (addon.js#mergeSubtitles)
 * converts to/from SRT timestamp strings around the call.
 *
 * Text-aware extensions
 * ---------------------
 * When cues carry `.text`, we build per-cue features (digit tokens, length,
 * punctuation flags — see lib/textFeatures.js) once at the start and pass
 * them into anchor selection and bipartite scoring. Text features provide:
 *
 *   • Tiebreaker — small bonus when timing is ambiguous in dense dialogue.
 *   • High-confidence promotion — a shared digit ("1985", "3:00") anchors
 *     a pair even when temporal proximity wouldn't.
 *   • Digit-mismatch penalty — pairs whose digits actively disagree
 *     ("1985" vs "2020") are rejected outright, regardless of timing.
 *
 * Cues without `.text` get empty feature objects and behavior reduces to
 * the original timing-only algorithm.
 */
...
function featAt(feats, i) {
  if (!feats || i < 0 || i >= feats.length) return EMPTY_FEATURES;
  return feats[i] || EMPTY_FEATURES;
}
...
function estimateOffsetMs(mainSubs, transSubs, options = {}) {
  ...
  //
  // The 1.02× margin (was 1.05×) is intentionally tight: dense-dialogue
  // shows like Rick and Morty have presence signals that are "on" almost
  // everywhere, so even a clearly wrong alignment gets a high lag=0
  // score and the offset shift only beats it by a few percent. The
  // multi-candidate selection in alignAndMatch is the safety net — if
  // we commit to a wrong offset here, that pass overrules us by match-
  // rate comparison.
  const maxPossible = Math.min(mainActive, transActive);
  if (bestScore < minConfidence * maxPossible) return 0;
  if (bestLag !== 0 && bestScore < zeroScore * 1.02) return 0;
...
function roundToStep(value, stepMs) {
  return Math.round(value / stepMs) * stepMs;
}

/**
 * Estimate a constant offset by pairing cues by their relative sequence
 * position. This catches dense-dialogue cases where cross-correlation can
 * prefer the neighboring cue because the "speaker active" signal overlaps
 * almost everywhere.
 *
 * Returns the offset to ADD to the secondary track's timestamps.
 */
function estimateSequenceOffsetMs(mainSubs, transSubs, options = {}) {
  const {
    maxOffsetMs = 30000,
    minCues = 8,
    maxLengthRatio = 1.8,
  ...
  if (!mainSubs || !transSubs || mainSubs.length < minCues || transSubs.length < minCues) {
    return 0;
  }

  const lengthRatio =
    Math.max(mainSubs.length, transSubs.length) / Math.max(1, Math.min(mainSubs.length, transSubs.length));
  if (lengthRatio > maxLengthRatio) return 0;

  const samples = Math.min(sampleCount, mainSubs.length, transSubs.length);
  if (samples < minCues) return 0;

  const deltas = [];
  for (let i = 0; i < samples; i++) {
    const mi = samples === 1
      ? 0
      : Math.round((i * (mainSubs.length - 1)) / (samples - 1));
    const ti = samples === 1
      ? 0
      : Math.round((i * (transSubs.length - 1)) / (samples - 1));
    const m = mainSubs[mi];
    const t = transSubs[ti];
    if (!m || !t || !Number.isFinite(m.startMs) || !Number.isFinite(t.startMs)) continue;
    deltas.push(m.startMs - t.startMs);
  }

  if (deltas.length < minCues) return 0;

  const offset = median(deltas);
  if (!Number.isFinite(offset)) return 0;
  if (Math.abs(offset) < minOffsetMs || Math.abs(offset) > maxOffsetMs) return 0;

  const mad = median(deltas.map(d => Math.abs(d - offset)));
  if (mad > maxMadMs) return 0;

  return roundToStep(offset, stepMs);
}
...
/**
 * Find anchor pairs (mainSub, transSub) for drift regression.
 *
 * Two paths into the anchor set:
 *
 *   1. Temporal-clear: among trans cues within `anchorThresholdMs` of the
 *      main cue's start, the single best candidate must beat the
 *      runner-up by 1.5× on a *combined* score = temporal proximity +
 *      text similarity. Text similarity acts as a tiebreaker in dense
 *      dialogue where two trans cues are nearly equidistant.
 *
 *   2. High-confidence text: a shared digit token (digitSim ≥ 0.5) is
 *      strong enough evidence to anchor a pair even when the temporal
 *      runner-up is close. This is the path that recovers alignment in
 *      dense scenes that pure timing can't resolve.
 *
 * @param {Array<{startMs:number, endMs:number}>} mainSubs
 * @param {Array<{startMs:number, endMs:number}>} transSubs
 * @param {number} anchorThresholdMs
 * @param {Array} [mainFeats] - parallel to mainSubs, from textFeatures
 * @param {Array} [transFeats] - parallel to transSubs, from textFeatures
 */
function findAnchorPairs(mainSubs, transSubs, anchorThresholdMs, mainFeats, transFeats) {
  const anchors = [];
  let j = 0;
  for (let mi = 0; mi < mainSubs.length; mi++) {
    const m = mainSubs[mi];
    const mFeat = featAt(mainFeats, mi);
    ...
    for (let k = j; k < transSubs.length; k++) {
      ...
      if (d > anchorThresholdMs) continue;

      const tFeat = featAt(transFeats, k);
      // Hard reject: digit sets contradict (e.g. "1985" vs "2020").
      if (digitsContradict(mFeat, tFeat)) continue;

      const temporal = 1 - d / anchorThresholdMs;          // 1..0
      const text = textSimilarity(mFeat, tFeat);            // 0..1
      const score = temporal + 0.5 * text;
      const highConf = isHighConfidenceTextMatch(mFeat, tFeat);
    ...
    // Anchor when EITHER the score is clearly better than the runner-up
    // (temporal-clear path) OR a shared digit makes this a near-certain
    // pairing regardless of how close the runner-up is.
    const clearWinner = bestScore > Math.max(secondScore, 0) * 1.5;
    if (clearWinner || bestHighConfidence) {
...
function estimateAffineMapping(mainSubs, transSubs, options = {}) {
  ...
  const anchors = findAnchorPairs(
    mainSubs, transSubs, anchorThresholdMs, mainFeats, transFeats
  );
 ...
 *
 * Text features add two corrections:
 *   • Small bonus (≤0.1) when text similarity is high — breaks ties in
 *     dense dialogue where two trans cues both overlap the same main.
 *   • Hard zero when digits actively contradict — "1985" can never pair
 *     with "2020" no matter how good the timing looks.
 */
function pairScore(m, t, threshold, mFeat, tFeat) {
  // Digit contradictions are a categorical reject.
  if (mFeat && tFeat && digitsContradict(mFeat, tFeat)) return 0;

  const textBonus = (mFeat && tFeat) ? 0.1 * textSimilarity(mFeat, tFeat) : 0;
  // High-confidence text match (shared digit token) is near-deterministic
  // evidence the cues describe the same line. We boost much more strongly
  // here so it can outvote a marginally-better timing overlap from a
  // non-matching peer cue.
  const highConfBoost =
    (mFeat && tFeat && isHighConfidenceTextMatch(mFeat, tFeat)) ? 0.5 : 0;

  const o = overlapScore(m, t);
  if (o > 0) return o + textBonus + highConfBoost;

  const startDiff = Math.abs(t.startMs - m.startMs);
  if (startDiff < threshold) return 0.001 + 1 / (1 + startDiff) + textBonus + highConfBoost;
  return 0;
}
...
function assignMatches(mainSubs, transSubs, options = {}) {
  ...
  for (let mi = 0; mi < mainSubs.length; mi++) {
    ...
    for (let ti = transStart; ti < transSubs.length; ti++) {
      ...
      const tFeat = featAt(transFeats, ti);
      const score = pairScore(m, t, threshold, mFeat, tFeat);
  ...
  for (const [mi, picked] of matches) {
    ...
    for (const dir of [1, -1]) {
      ...
      while (
        ...
        // Sentence-boundary wall — never absorb across a hard sentence
        // terminator. Without this the picked set can swallow an
        // adjacent trans cue from the previous/next sentence (which is
        // how "Couldn't you do this an hour ago?  I was eating, and
        // you're dirty." ends up duplicated under one main cue).
        //
        // Going right: the wall sits at the last absorbed cue (or anchor).
        // Going left:  the wall sits at the candidate itself, because
        //              absorbing it would mean stepping past its end.
        const wallFeat = dir === 1 ? featAt(transFeats, prev) : tFeat;
        if (wallFeat.endsSentence) break;

        // Don't absorb cues that contradict the main on digits — those
        // are different lines that just happen to be adjacent.
        if (digitsContradict(mFeat, tFeat)) break;
...
function alignAndMatch(mainSubs, transSubs, options = {}) {
  ...
  // Build text-feature arrays once. When cues have no `.text`, the arrays
  // are full of empty-feature objects and downstream code reduces to the
  // original timing-only behavior.
  const mainFeats = enableTextFeatures ? buildFeatureArray(mainSubs) : null;
  const transFeats = enableTextFeatures ? buildFeatureArray(transSubs) : null;

  // applyOffset/applyAffine/applyLocalOffsets preserve cue indices, so the
  // feature arrays stay parallel even as `trans` mutates downstream.
  let trans = transSubs;
  let offsetMs = 0;
  let drift = null;
  let localAnchors = [];
  ...
  if (enableOffset && trans.length > 0 && mainSubs.length > 0) {
    // Try multiple candidate offsets and pick the one that actually
    // yields the highest match count. The old gated if/else missed the
    // common "off-by-one cue" failure mode where the trans track is
    // shifted by ~3s, signal estimation is too timid to commit, and
    // sequence estimation is gated out because match count is still
    // high (every trans cue still pairs with *some* main cue, just the
    // wrong one).
    //
    // Tie-break: `0` is the first candidate and we only replace it on
    // STRICT improvement. This protects correctly-aligned tracks from
    // being nudged by candidate offsets that happen to score equal.
    const signalOffsetMs = estimateOffsetMs(mainSubs, trans);
    const sequenceOffsetMs = estimateSequenceOffsetMs(mainSubs, trans);

    const candidateOffsets = [0];
    if (signalOffsetMs !== 0) candidateOffsets.push(signalOffsetMs);
    if (sequenceOffsetMs !== 0 && !candidateOffsets.includes(sequenceOffsetMs)) {
      candidateOffsets.push(sequenceOffsetMs);
    }

    let bestScore = -1;
    let bestOffset = 0;
    for (const cand of candidateOffsets) {
      const candidateTrans = cand === 0 ? trans : applyOffset(trans, cand);
      const m = assignMatches(mainSubs, candidateTrans, {
        threshold: matchThreshold,
        allowMultiTrans,
        mainFeats,
        transFeats
      });
      // Strict `>` keeps `0` as the winner on ties — see comment above.
      if (m.size > bestScore) {
        bestScore = m.size;
        bestOffset = cand;
      }
    ...
    if (offsetMs !== 0) {
      ...
      log(
        `syncEngine: applied global offset ${offsetMs}ms ` +
        `(tried ${candidateOffsets.length} candidates: ` +
        `signal=${signalOffsetMs}, sequence=${sequenceOffsetMs})`
      );
    }
  }
