...
function alignAndMatch(mainSubs, transSubs, options = {}) {
  ...
  if (enableOffset && trans.length > 0 && mainSubs.length > 0) {
    const baselineMatches = assignMatches(mainSubs, trans, {
      threshold: matchThreshold,
      allowMultiTrans
    });
    const baselineMatchRate = mainSubs.length > 0 ? baselineMatches.size / mainSubs.length : 0;
    const signalOffsetMs = estimateOffsetMs(mainSubs, trans);
    const sequenceOffsetMs = estimateSequenceOffsetMs(mainSubs, trans);
    offsetMs = signalOffsetMs;

    if (
      sequenceOffsetMs !== 0 &&
      baselineMatchRate < 0.5 &&
      (signalOffsetMs === 0 || Math.abs(sequenceOffsetMs - signalOffsetMs) > matchThreshold)
    ) {
      offsetMs = sequenceOffsetMs;
      log(
        `syncEngine: preferred sequence offset ${sequenceOffsetMs}ms ` +
        `(signal offset ${signalOffsetMs}ms, baseline ${(baselineMatchRate * 100).toFixed(1)}%)`
      );
    }
