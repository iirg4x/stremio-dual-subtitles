...
const { fetchJimakuSubtitles } = require('./animeSource');
...
const SUBTITLE_SOURCES = [
  ...
  },
  {
    id: 'jimaku',
    name: 'Jimaku',
    website: 'https://jimaku.cc/',
    type: 'Anime subtitle database',
    notes: 'High-quality Japanese anime subtitle database keyed by AniList ID. Also serves English fansub tracks. Optional API key unlocks higher rate limits.',
    status: 'optional',
    envKey: null,
    enabledByDefault: true,
    adapter: 'jimaku',
    capabilities: ['anilist-id', 'kitsu-id', 'movie', 'series', 'anime']
  },
  {
    id: 'subdl',
    name: 'SubDL',
    website: 'https://subdl.com/',
    type: 'Subtitle database with API',
    notes: 'Large multilingual subtitle database. Free API key available at subdl.com/app/key.',
    status: 'optional',
    envKey: 'SUBDL_API_KEY',
    enabledByDefault: false,
    adapter: 'subdl',
    capabilities: ['imdb-id', 'movie', 'series', 'filename']
  },
  {
    id: 'addic7ed',
    name: 'Addic7ed',
...
function isSubtitleSourceEnabled(source, env = process.env, context = {}) {
  if (!source) return false;
  // Jimaku: always enabled but only useful when anime context is present.
  // We still include it in the enabled list; fetchJimakuFromContext will
  // return [] quickly when kitsuId/anilistId are absent.
  if (source.id === 'jimaku' && source.enabledByDefault) return true;
...
function fetchFromSource(source, context) {
  ...
  if (source.id === 'jimaku') return fetchJimakuFromContext(context);
  if (source.id === 'subdl') return fetchSubDL(context);
  return Promise.resolve([]);
}

async function fetchJimakuFromContext(context) {
  // Only query Jimaku when we have anime-relevant context
  const { kitsuId, anilistId, episode, fetchWithRetry, env } = context;
  if (!kitsuId && !anilistId) return [];
  return fetchJimakuSubtitles({ fetchWithRetry, kitsuId, anilistId, episode, env });
}

const SUBDL_API = 'https://api.subdl.com/api/v1/subtitles';

async function fetchSubDL(context) {
  const { fetchWithRetry, imdbId, type, season, episode, languages, videoParams, env } = context;
  const apiKey = (env || process.env).SUBDL_API_KEY;
  if (!apiKey || !imdbId) return [];

  const params = new URLSearchParams();
  params.set('api_key', apiKey);
  params.set('imdb_id', `tt${normalizeImdbId(imdbId)}`);
  if (type === 'movie') params.set('type', 'movie');
  else params.set('type', 'tv');
  if (season) params.set('season_number', String(season));
  if (episode) params.set('episode_number', String(episode));

  const langList = (languages || [])
    .map(l => toSubDLLanguageCode(l))
    .filter(Boolean)
    .join(',');
  if (langList) params.set('languages', langList);
  if (videoParams && videoParams.filename) params.set('film_name', videoParams.filename);

  try {
    const res = await fetchWithRetry(`${SUBDL_API}?${params}`, { timeout: 12000 });
    const subs = (res && res.data && Array.isArray(res.data.subtitles))
      ? res.data.subtitles
      : [];
    return subs.map(sub => normalizeSubDLSubtitle(sub)).filter(Boolean);
  } catch (_) {
    return [];
  }
}
...
const SUBDL_LANG_MAP = {
  ...
  tha: 'TH', ind: 'ID', may: 'MS', msa: 'MS', per: 'FA', fas: 'FA'
};

function toSubDLLanguageCode(lang) {
  return SUBDL_LANG_MAP[lang] || null;
}

function normalizeSubDLSubtitle(sub) {
  if (!sub) return null;
  // SubDL returns subtitles with a `url` that is either a direct file URL or
  // a download page. The `subtitleLink` or `download` fields have the direct link.
  const url = sub.subtitleLink || sub.download || sub.url;
  if (!url) return null;
  const fullUrl = url.startsWith('http') ? url : `https://dl.subdl.com${url}`;
  const lang = toOpenSubtitlesLanguageCode3(sub.lang || sub.language || '');
  if (!lang) return null;

  const release = sub.releaseName || sub.release_name || sub.name || '';
  return {
    id: `subdl-${sanitizeId(sub.subtitleId || sub.subtitle_id || url)}`,
    url: fullUrl,
    lang,
    SubEncoding: 'UTF-8',
    m: 'i',
    g: releaseToGroupKey(release),
    downloads: Number(sub.downloads || sub.download_count || 0),
    _sourceId: 'subdl',
    _sourceName: 'SubDL',
    _release: release,
    _fileName: sub.name || null
  };
}

// SubDL returns 2-letter codes; convert back to 3-letter OS codes
const SUBDL_REVERSE_MAP = {};
for (const [k, v] of Object.entries(SUBDL_LANG_MAP)) {
  if (!SUBDL_REVERSE_MAP[v]) SUBDL_REVERSE_MAP[v] = k;
}

function toOpenSubtitlesLanguageCode3(code) {
  if (!code) return null;
  const upper = code.toUpperCase();
  return SUBDL_REVERSE_MAP[upper] || toOpenSubtitlesLanguageCode(code);
}
